...
<%
	...
	if(userID == null){
		...
		script.println("alter('로그인을 해주세요.');");
	...
	if(emailChecked == true){
		PrintWriter script = response.getWriter();
		script.println("<script>");
		script.println("alter('이미 인증된 회원입니다.')");
		script.println("location.href ='index.jsp';");
		script.println("</script>");
		script.close();
		return;
	}
	String host = "http://localhost:9090/Lecture_Evaluation/";
	String from = "a01046528608@gmail.com";
	...
	try{
		...
		e.printStackTrace();
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alter('오류가 발생했습니다.')");
